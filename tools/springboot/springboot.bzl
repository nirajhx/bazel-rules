...
def springboot(name, java_library, boot_app_class, deps, fail_on_duplicate_classes = False, tags = []):
    ...
    native.genrule(
        ...
        cmd = "$(location @shared_bazel_rules//tools/springboot:write_manifest.sh) " + boot_app_class + " $@ $(SRCS)",
              message = "SpringBoot rule is writing the MANIFEST.MF...",
        tools = ["@shared_bazel_rules//tools/springboot:write_manifest.sh"],
    ...
    native.genrule(
        ...
        cmd = "$(location @shared_bazel_rules//tools/springboot:write_gitinfo_properties.sh) $@",
        tools = ["@shared_bazel_rules//tools/springboot:write_gitinfo_properties.sh"],
        outs = [gengitinfo_out],
        tags = tags,
        stamp = 1,
    )
    ...
    native.genrule(
        ...
        cmd = "$(location @shared_bazel_rules//tools/springboot:springboot_pkg.sh) " +
              "$(location @bazel_tools//tools/jdk:singlejar) " +
              boot_app_class + " " + verify_str + " $(JAVABASE) " + name + " $@ $(SRCS)",
        tools = [
            "@shared_bazel_rules//tools/springboot:springboot_pkg.sh",
            "@shared_bazel_rules//tools/springboot:verify_conflict.py",
            "@shared_bazel_rules//tools/springboot:whitelist.txt",
